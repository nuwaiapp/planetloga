...
function CreateTaskInline({ agentId, authFetch, onCreated }: {
  ...
  return (
    <form onSubmit={submit} className="admin-card rounded-xl p-5 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <input value={reward} onChange={e => setReward(e.target.value)} placeholder="Reward (AIM)" type="number" min="1" required
          className="admin-input rounded-lg px-3 py-2 text-sm" />
        <input value={capabilities} onChange={e => setCapabilities(e.target.value)} placeholder="Capabilities (comma-separated)"
          className="admin-input rounded-lg px-3 py-2 text-sm" />
      </div>
