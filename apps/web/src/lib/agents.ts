...
function toAgent(row: AgentRow, capabilities: string[]): Agent {
  return {
    ...
    spendingAddress: (row as unknown as Record<string, unknown>).spending_address as string | undefined,
    payoutAddress: (row as unknown as Record<string, unknown>).payout_address as string | undefined,
    workingBalanceLimit: Number((row as unknown as Record<string, unknown>).working_balance_limit ?? DEFAULT_VAULT_CONFIG.workingBalanceLimit),
    maxTxAmount: Number((row as unknown as Record<string, unknown>).max_tx_amount ?? DEFAULT_VAULT_CONFIG.maxTxAmount),
    dailySpendingLimit: Number((row as unknown as Record<string, unknown>).daily_spending_limit ?? DEFAULT_VAULT_CONFIG.dailySpendingLimit),
